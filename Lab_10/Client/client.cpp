...
int main() {
    ...
    if (publicKey == nullptr) {
        std::cerr << "Ошибка загрузки ключа." << std::endl;
        return 1;
    }
    ...
    SHA256(reinterpret_cast<const unsigned char*>(credentials.c_str()), credentials.size(), hash);

    std::time_t current_time = std::time(nullptr);
    //std::time_t current_time = std::time(nullptr) - 90;
    std::string payload(reinterpret_cast<char*>(hash), SHA256_DIGEST_LENGTH);
    payload.append(reinterpret_cast<char*>(&current_time), sizeof(current_time));
    ...
    if (EVP_PKEY_encrypt(ctx, nullptr, &encrypted_len, reinterpret_cast<const unsigned char*>(payload.c_str()), payload.size()) <= 0) {
    ...
    if (EVP_PKEY_encrypt(ctx, encrypted_data, &encrypted_len, reinterpret_cast<const unsigned char*>(payload.c_str()), payload.size()) <= 0) {
        std::cerr << "Ошибка шифрования: " << ERR_error_string(ERR_get_error(), nullptr) << std::endl;
        EVP_PKEY_CTX_free(ctx);
        cleanup(client_socket);
        return 1;
